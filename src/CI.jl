...
function RunUCI(MolInAng::Vector{Atom}, Charge::Int, Multiplicity::Int, MaxExcitation::Int; MaxIter = 128, Threshold = 1e-8)
	...
	isnothing(CI_Results) && error("CI calculation did not converge. Aborting.")

	print_ci_analysis(CI_Results)
	print_job_time(TStart, "UHF-CI")
...
function RunRCI(MolInAng::Vector{Atom}, Charge::Int, Multiplicity::Int, MaxExcitation::Int)
	...
	isnothing(RHF_Res) && error("RHF calculation did not converge. Aborting.")
